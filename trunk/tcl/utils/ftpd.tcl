...
#       subcommand and arguments that are passed to it. The full path is sent back.
#
...
proc ::ftpd::command::PASV {sock list} {
    ...
    puts $sock "500 You're firewalled? What a pity..."
    ::ftpd::Log debug "->500 You're firewalled? What a pity..."
...
proc ::ftpd::command::PWD {sock list} {
    ...
    ::ftpd::Log debug "->257 \"$data(cwd)\" is current directory."
    return
}
...
proc ::ftpd::command::SIZE {sock list} {
    ...
    # a trimleft
    #set path [file join $data(cwd) [string trimleft [file tail $filename] /]]
	set path [::ftpd::create_path $data(cwd) $filename]
...
proc ::ftpd::GetDone {sock sock2 f filename bytes {err {}}} {
    ...
    ::ftpd::Log debug "GetDone $f $sock2 $bytes bytes filename: $filename"
    return
}
	...
	dlist {
		...
		list {
			...
			file stat $f stat
			    ...
			    ::ftpd::Log debug "->[format "%s %3d %s %8s %11s %s %s" \
			            [PermBits $f $stat(mode)] $stat(nlink) \
	                            $user $group $stat(size) \
                                    [FormDate $stat(mtime)] [file tail $f]]"
                        } else {
                            puts $outchan [format "%s %3d %11s %s %s" \
                                    ...
                                    [file tail $f]]
                            ::ftpd::Log debug "->[format "%s %3d %11s %s %s" \
                                    [PermBits $f $stat(mode)] $stat(nlink) \
                                    $stat(size) [FormDate $stat(mtime)] \
                                    [file tail $f]]"
                        }
