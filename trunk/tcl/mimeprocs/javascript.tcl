...
namespace eval js {
		...
		if { ![catch {set action [fas_get_value action] } ] } {
			if { $action != "view" } {
				...
				if { [set ${action}::done ] == 0 } {
					fas_debug "javascript::new_type - action -> $action , action::done -> [set ${action}::done]"
	...
	proc get_title { filename } {
		...
		fas_debug "js::get_title found ->$title<-"
		return $title
	}
	...
	#proc content_display { current_env content } {
		...
		#binary::content_display js $content
		...
		#	global conf
		#	set sock $conf(sock)
		#	Httpd_ReturnData $sock [js::mimetype] "$content"
		#}
	#}
	...
	proc display { current_env filename } {
		...
		upvar $current_env fas_env
		global FAS_VIEW_URL
		if { [catch {open $filename} js_id] } {
			puts ""
		} else {
			set js_content [read $js_id]
			close $js_id
			set final_js_content [string map [list "fas:" "${FAS_VIEW_URL}?file="] $js_content]
			::not_binary::content_display js $final_js_content
		 }
	...
	proc content { current_env filename } {
		# A procedure for just displaying the file in html directly
		upvar $current_env fas_env
		return "[not_binary::content fas_env $filename js]"
