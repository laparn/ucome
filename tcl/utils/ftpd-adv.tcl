...
# Current limitation : filename with spaces are not handled correctly
# for example with the STOR command (transfer of a file to the ftp server)
#
# Trying to handle PASV. For that I add a PASV flag in the $sock
# structure. During PASV, I also try to determinate a "free" socket
# to use. Then, for events (reading or writing on this socket), I create a call
# back on the socket
# for it to be ready to send a file (the name of which is stored in $sock),
# or receive a file when asked.
# Knowledge extracted from RFC 959 and from 2 articles of http://slacksite.com
# /other/ftp.html and http://slacksite.com/other/ftp-appendix1.html
...
namespace eval ::ftpd {
    ...
    variable cfg
    if {![info exists cfg]} {
        #::ftpd::Log debug "::ftpd - initializing cfg"
	array set cfg [list \
	    closeCmd  {} \
	    authIpCmd  {} \
	...
	array set commands [list \
	    ...
	    PASS       {PASS <sp> password} \
             PASV       {PASV (set passive transfer mode)} \
...
proc ::ftpd::config {args} {
	...
	{pasv_support.arg {1} {Enable or disable passive transfer support.}}]]
    return
}
...
proc ::ftpd::server {{myaddr {}}} {
    variable port
	::ftpd::Log debug "::ftpd::server : trying to open a socket..."
    if {[string length $myaddr]} {

		if { [ catch {socket -server ::ftpd::accept -myaddr $myaddr $port} f ] } {
			Log error "::ftpd::server : $f"
			return
		}
    } else {
		if { [ catch {socket -server ::ftpd::accept $port} f ] } {
			Log error "::ftpd::server : $f"
			return
		}
    }
    set port [lindex [fconfigure $f -sockname] 2]
	Log debug "::ftpd::server : socket opened with success on port $port."
    return
}
...
proc ::ftpd::accept {sock ipaddr client_port} {
	...
	::ftpd::Log debug "::ftpd::accept on $ipaddr $client_port"
    ...
    ::ftpd::Log debug "->220 $welcome"
    ::ftpd::Log debug "Accept $ipaddr"
    puts "Accept $ipaddr"
    return
}
...
proc ::ftpd::read {sock} {
	...
	command {
	    ...
	    auto_load ::ftpd::command::$cmd
		    ...
		    ::ftpd::Log debug "->503 Login with USER first."
                } else {
                    puts $sock "530 Please login with USER and PASS."
                    ::ftpd::Log debug "->530 Please login with USER and PASS."
		}
	    ...
	    ::ftpd::Log debug $command
		::ftpd::command::$cmd $sock [lrange $parts 1 end]
		catch {flush $sock}
	    } else {
	    ::ftpd::Log error "Unknown command: $cmd"
		puts $sock "500 Unknown command $cmd"
	    ::ftpd::Log debug "500 Unknown command $cmd"
	    }
	}
	default {
...
#       command is recieved.
#       If in PASV mode and coming from data_read, I do not close
#       the channel.
#
# Arguments:
#       sock -                   The channel for this connection to the ftpd.
...
proc ::ftpd::FinishData {sock} {
    ...
    ::ftpd::Log debug "::ftpd::FinishData - entering - data(sock2) is $data(sock2)"
	if { $data(sock2) != {} } {
    catch {close $data(sock2)}
    ::ftpd::Log debug "::ftpd::FinishData - closing $data(sock2)"
    set data(sock2) {}
	}

    # For pasv case, I close also the sock2a.
    # No more pasv situation.
	set data(sock2a) {}
	set data(data_sock_nber) {}

    variable last_port
    catch { incr last_port -1 }
    return
}

#
# Arguments :
#       cwd : the current directory where we are
...
proc ::ftpd::command::ABOR {sock list} {
    ...
    ::ftpd::Log debug "->225 ABOR command successful."
...
proc ::ftpd::command::APPE {sock list} {
	...
	set path [::ftpd::create_path $data(cwd) $filename]
    ...
    if {[::ftpd::hasCallback authFileCmd]} {
	    ...
	    ::ftpd::Log debug "->550 $filename: Permission denied"
            return
        }
    }
	...
	puts $sock "150 Copy Started ($data(mode))"
        ::ftpd::Log debug "150 Copy Started ($data(mode))"

		# modif Xav
		if { $data(sock2) != "" } {
	fcopy $data(sock2) $f -command [list ::ftpd::GetDone $sock $data(sock2) $f ""]
		} else {
			puts $sock "520 Data channel is not open yet."
			::ftpd::Log debug "->520 Data channel is not open yet."
		}
    } else {
    puts $sock "500 Copy Failed: $filename $f"
    ::ftpd::Log debug "500 Copy Failed: $filename $f"
	::ftpd::FinishData $sock
    }
    return
}
...
proc ::ftpd::command::CDUP {sock list} {
    ...
    ::ftpd::Log debug "->200 CDUP command successful."
    return
}
...
proc ::ftpd::command::DELE {sock list} {
	...
	::ftpd::Log debug "500 DELE Failed: $path $msg"
    }
    return
}
...
proc ::ftpd::command::HELP {sock list} {
    ...
    if {[llength $list] > 0} {
        ...
        ::ftpd::Log debug "214 Direct comments to $::ftpd::contact."
    }
...
proc ::ftpd::command::LIST {sock list} {
    # There is a special mc case. With mc, when I connect
    # and try to have the directory list after the connection
    # cd /#ftp:admin@localhost:7777
    # I have the following debug :
    # debug     Accept 127.0.0.1
    # debug     USER admin
    # debug     user <admin>
    # debug     PASS admin
    # debug     pass <admin>
    # debug     PWD
    # debug     
    # error     Unknown command: PASV
    # debug     PORT 127,0,0,1,133,37
    # debug     TYPE A
    # debug     LIST -la /.
    # debug     ::ftpd::List - Fs dlist -la list sock5
    # As you see, mc adds a -la option. It breaks ftpd
    # that sends nothing. So I am going to test
    # for the first argument, if it is -la, I take the next
    # argument and remove the leading /
    # And there is a special konqueror case as well :
    # list -a is the command
    ::ftpd::Log debug "Entering ::ftpd::command::LIST"
    set argument [lindex $list 0]
    # Konqueror case
    ::ftpd::Log debug "::ftpd::command::LIST list is $list"
    switch -exact -- $argument {
    	"-a" {
    	   ::ftpd::Log debug "::ftpd::command::LIST KONQUEROR case"
    	   if { [llength $list] == 1 } {
               set filename ""
           } else {
   	           set filename [string trimleft [lindex $list 1] "/"]
           }
           ::ftpd::Log debug "::ftpd::command::LIST filename is ->${filename}<-"
        }
        "-la" {
            # MC case LIST -la /.
    	    ::ftpd::Log debug "::ftpd::command::LIST mc case"
    	    set filename [string trimleft [lindex $list 1] "/"]
        } 
        default {
...
proc ::ftpd::command::MKD {sock list} {
	...
	::ftpd::Log debug "->500 MKD Failed: $path $f"
    }
    return
}
...
proc ::ftpd::command::NOOP {sock list} {
    ...
    ::ftpd::Log debug "->200 NOOP command successful."
    return
}
...
proc ::ftpd::command::PASS {sock list} {
	...
	::ftpd::Log error "-authUsrCmd must not be empty, need a way to authenticate the user."
    }
	...
	unset data(user)
		...
		::ftpd::Log debug "->551 Access Denied"
    } else {
	puts $sock "230 OK"
	::ftpd::Log debug "-> 230 OK"
	set data(access) 1
    }
    return
}
...
proc ::ftpd::command::PORT {sock list} {
	...
	puts "::command::PORT sock is $sock"
	puts "::command::PORT list is $list"
	puts "::command::PORT x is $x"

	#in the active mode, it's the server which sends data. So
	#the PORT command sent by the client forces the server to
	#reopen a socket on the new client port. But its own port
	#number musn't change.
	#when in the passive mode, if the client sends a PORT command
	#then the transfer mode is switched to active.

	# begin modif Xav
	# eventually switch from passive to active transfers
	if { $data(pasv_flag) } {
		set $data(pasv_flag) 0
	}
	# end modif Xav
	...
	# modif Xav : $list must be a six elements (bytes) list
	if { [llength $x] == 6 } {
    ...
    ::ftpd::Log debug "->200 PORT $list OK"
	} else {
		puts $sock "501 INVALID SYNTAX"
		::ftpd::Log debug "->501 INVALID SYNTAX : PORT $list"
		::ftpd::Log note "::ftpd::command::PORT $list : data(sock2) is $data(sock2)"
	}
    return
}
...
proc ::ftpd::command::PWD {sock list} {
    ...
    puts $sock "257 \"$cwd\" is current directory."
    ::ftpd::Log debug "->257 \"$cwd\" is current directory."
    return
}
...
proc ::ftpd::command::RETR {sock list} {
    ...
    set filename [lindex $list 0]
	# KONQUEROR case there was a cwd before and they put twice the
	# path in cwd and in the filename => I must take the tail of filename
	# MC case : CWD is not used but the file as the path --- GRRRR !!!
	#if { $data(cwd) == "" } {
		# MC
	#	set filename_to_use $filename
	#} else {
		# KONQUEROR case
	#	set filename_to_use [file tail $filename]
	#}
    #set path [file join $data(cwd) [string trimleft $filename_to_use /]]
    ...
    # Patched Mark O'Connor
    #
    if {![catch {::ftpd::Fs retr $path $data(mode) $data(sock2)} f]} {
	puts $sock "150 Copy Started ($data(mode))"
	::ftpd::Log debug "->150 Copy Started ($data(mode))"
	...
	fcopy $f $data(sock2) -command [list ::ftpd::GetDone $sock $data(sock2) $f $filename]
	}
    } else {
	puts $sock "500 Copy Failed: $path $f"
	::ftpd::Log debug "->500 Copy Failed: $path $f"
	::ftpd::FinishData $sock
    }
    return
}
...
#       Only applies on an empty directory !
#
# Arguments:
#       sock -                   The channel for this connection to the ftpd.
...
proc ::ftpd::command::RMD {sock list} {
	...
	puts $sock "500 RMD Failed: $path $f"
		::ftpd::Log debug "->500 RMD Failed: $path $f"
    }
    return
}


## Recursive DELE+RMD command.
## Applies on every directories, even the not-empty ones.
proc ::ftpd::command::XRMD {sock list} {
    upvar #0 ::ftpd::$sock data
    ...
    if {[catch {::ftpd::Fs xrmd $path $sock} f]} {
		puts $sock "500 XRMD Failed: $path $f"
		::ftpd::Log debug "->500 XRMD Failed: $path $f"
    }
    return
}
...
proc ::ftpd::command::RNFR {sock list} {
    ...
    if {[file exists $path]} {
       ...
       ::ftpd::Log debug "->350 File exists, ready for destination name"
        set data(renameFrom) $path
    } else {
        puts $sock "550 $path: No such file or directory."
        ::ftpd::Log debug "->550 $path: No such file or directory."
    }
    return
}
...
proc ::ftpd::command::RNTO {sock list} {
    ...
    if {[llength $list] == 0} {
		...
		::ftpd::Log debug "->500 'RNTO': command not understood."
        return
    }
    set filename [lindex $list 0]
	set path [::ftpd::create_path $data(cwd) $filename]
    ...
    if {![info exists data(renameFrom)]} {
		...
		::ftpd::Log debug "->503 Bad sequence of commands."
        return
    }
    if {[::ftpd::hasCallback authFileCmd]} {
        set cmd $::ftpd::cfg(authFileCmd)
...
proc ::ftpd::command::SIZE {sock list} {
    ...
    set filename [lindex $list 0]
    # In the case of KONQUEROR the full relative path to the root
    # directory of the file is sent, after a CWD then
    # you have a dir /tmp/ucometest/any/sc/any/sc => which is
    # bad. Then I must use the tail of the file not only
    # a trimleft
	set path [::ftpd::create_path $data(cwd) $filename]
    #    set path [file join $data(cwd) [string trimleft $filename /]]
    if {[catch {::ftpd::Fs size $path $sock} msg]} {
	puts $sock "500 SIZE Failed: $path $msg"
	::ftpd::Log debug "->500 SIZE Failed: $path $msg"
	::ftpd::FinishData $sock
    }
    return
}
...
proc ::ftpd::command::STOR {sock list} {
	...
	# begin modif Xav
	if { $data(sock2) != "" } {
		#::ftpd::Log debug "::ftpd::command::STOR data(mode) is $data(mode)"
		#::ftpd::Log debug "::ftpd::command::STOR sock is $sock"
		#::ftpd::Log debug "::ftpd::command::STOR data(sock2) is $data(sock2)"

		if { !$data(pasv_flag) } {
	
		# modif Xav {::ftpd::Fs store $path $data(mode)}
		::ftpd::Log debug "::ftpd::command::STOR active transfer mode"

    if {![catch {::ftpd::Fs store $path $data(mode) $data(sock2)} f]} {
	puts $sock "150 Copy Started ($data(mode))"
	::ftpd::Log debug "->150 Copy Started ($data(mode))"
	fcopy $data(sock2) $f -command [list ::ftpd::GetDone $sock $data(sock2) $f ""]
    } else {
	...
	::ftpd::FinishData $sock
			...
			# I am in PASV mode WARNING
			# So I will store the filename
			::ftpd::Log debug "::ftpd::command::STOR - storing pasv_filename -: $path in data array"
			...
			# begin modif Xav
			#puts $sock "150 Copy Started ($data(mode))"
			#::ftpd::Log debug "::ftpd::command::STOR ->150 Copy Started ($data(mode))"
			#set data(pasv_filename) $path
			# end modif Xav
		}
	} else {
		puts $sock "520 Data channel is not open yet."
...
proc ::ftpd::command::STOU {sock list} {
    ...
    # Patched Mark O'Connor
    #
	# modif Xav ::ftpd::Fs store $file $data(mode)
    if {![catch {::ftpd::Fs store $file $data(mode) $data(sock2)} f]} {
	puts $sock "150 Copy Started ($data(mode))"
    ::ftpd::Log debug "->150 Copy Started ($data(mode))"
...
proc ::ftpd::command::SYST {sock list} {
    ...
    ::ftpd::Log debug "->215 $platform Type: L8 Version: $version"
...
proc ::ftpd::command::TYPE {sock list} {
    ...
    ::ftpd::Log debug "->200 Type set to $type."
    return
}
...
proc ::ftpd::command::USER {sock list} {
    ...
    ::ftpd::Log debug "->331 Password Required"
...
proc ::ftpd::GetDone {sock sock2 f filename bytes {err {}}} {
	::ftpd::Log note "into GetDone $sock $sock2 $f $filename"
    upvar #0 ::ftpd::$sock data
    variable cfg

    close $f
    FinishData $sock
	::ftpd::Log note "FinishData complete"

    if {[string length $err]} {
	puts $sock "226- $err"
    } elseif {$filename == ""} {
        puts $sock "226 Transfer complete ($bytes bytes)"
        ::ftpd::Log debug "->226 Transfer complete ($bytes bytes)"
    } else {
        puts $sock "226 Transfer complete (unique file name: $filename)."
        ::ftpd::Log debug "->226 Transfer complete (unique file name: $filename)."
    }
    if {[hasCallback xferDoneCmd]} then {
	catch {$cfg(xferDoneCmd) $sock $sock2 $f $bytes $filename $err}
    }
    ::ftpd::Log note "GetDone $f $sock2 $bytes bytes filename: $filename"
    return
}
...
proc ::ftpd::List {sock filename style} {
    ...
    ::ftpd::Log debug "->150 Opening data channel"

    set path [file join $data(cwd) $filename]
    ::ftpd::Log debug "::ftpd::List filename is ->${filename}<- style is $style"

	# modif Xav : the data channel must be opened
	if { $data(sock2) != "" } {
    	::ftpd::Fs dlist $path $style $data(sock2)
	} else {
		puts $sock "520 Data channel is not open yet."
		::ftpd::Log debug "->520 Data channel is not open yet."
	}
    ...
    ::ftpd::Log debug "->226 Listing complete"
    return
}
...
proc ::ftpd::fsFile::fs {command path args} {
	...
	dlist {
	    ...
	    ::ftpd::Log note "at dlist {$style} {$outchan} {$path}"
	    ...
	    foreach {outchan} $args break
            ...
            if {![file exists $path]} {
				...
				::ftpd::Log debug "->550 $path: File exists."
	    } else {
                file delete $path
                puts $outchan "250 DELE command successful."
				::ftpd::Log debug "->250 DELE command successful."
	    }
	}
        exists {
            if {[file isdirectory $path]} {
	    ...
	    foreach {outchan} $args break
            ...
            if {[file exists $path]} {
                if {[file isdirectory $path]} {
					...
					::ftpd::Log debug "->521 \"$path\" directory exists"
		} else {
		    puts $outchan "521 \"$path\" already exists"
					::ftpd::Log debug "->521 \"$path\" already exists"
                }
	    } elseif {[file exists [file dirname $path]]} {
                file mkdir $path
                puts $outchan "257 \"$path\" new directory created."
				::ftpd::Log debug "->257 \"$path\" new directory created."
	    } else {
                puts $outchan "550 $path: No such file or directory."
					::ftpd::Log debug "->550 $path: No such file or directory."
	    }
	    ...
	    foreach {outchan} $args break
			...
			::ftpd::Log debug "->550 $path: not a plain file."
            } else {
                set time [file mtime $path]
                puts $outchan [clock format $time -format "213 %Y%m%d%H%M%S"]
				::ftpd::Log debug "->[clock format $time -format "213 %Y%m%d%H%M%S"]"
	    }
        }
        permissions {
	    file stat $path stat
        ...
        rename {
	        ...
	        puts $outchan "550 rename: No such file or directory."
				::ftpd::Log debug "->550 rename: No such file or directory."
            }
            file rename $path $newname
            puts $sock "250 RNTO command successful."
			::ftpd::Log debug "->250 RNTO command successful."
	}
        rmdir {
	    foreach {outchan} $args break
		...
		xrmd {
			foreach {outchan} $args break
				...
				if {![file isdirectory $path]} {
					...
					foreach filename [file join $path *] {
						if { [file isdirectory "$path/$filename"] } {
							::ftpd::fsFile::fs xrmd "$path/$filename" $args
						} elseif { [file isfile "$path/$filename"] } {
							::ftpd::fsFile::fs delete "$path/$filename" $args
						}
					}
				} else {
					file delete $path
					puts $outchan "250 XRMD command successful."
					::ftpd::Log debug "->250 XRMD command successful."
				}
		}
	    ...
	    foreach {outchan} $args break
	        ...
	        puts $outchan "550 $path: not a plain file."
				...
				::ftpd::Log debug "->213 [file size $path]"
	    }
        }
	default {
...
# ::ftpd::create_path --
#
#       Depending on the tool providing a path there are different cases
#       Normally the path is the concatenation of cwd and filename
#       However, for KONQUEROR there is a dupplication of the path in
#       cwd and filename. This must be handled
#
proc ::ftpd::create_path { cwd filename } {
	# KONQUEROR case there was a cwd before and they put twice the
	# path in cwd and in the filename => I must take the tail of filename
	# MC case : CWD is not used but the file as the path --- GRRRR !!!
	if { $cwd == "" } {
    ...
    set path [file join $cwd [string trimleft $filename_to_use /]]
	return $path
}
...
# ::ftpd::command::PASV --
#
#       Handle the PASV ftp command.  I just send back a 500 error
#
# Arguments:
#       sock -                   The channel for this connection to the ftpd.
#       list -                   The arguments to the PASS command.
...
# Side Effects:
#
proc ::ftpd::command::PASV {sock list} {
    upvar #0 ::ftpd::$sock data
    variable cfg

	if { !$::ftpd::cfg(pasv_support) } {
		puts $sock "502 PASV Command not implemented"
		::ftpd::Log debug "->502 PASV Command not implemented"
		::ftpd::Log debug "pasv - I send back an error"
	} else {
		# was a data socket previously given ?
		if { [info exists $data(pasv_flag)] } {
			if { $data(pasv_flag) } {
				#there is no need to find a new socket.
				#I send the existing one
				set current_sock $data(data_sock_nber)
				::ftpd::Log debug "::ftpd::command::PASV - PASVmode = $cfg(PASVmode), sock_number -> $data(data_sock_nber)"
				set ip $::ftpd::ip
				regsub -all {\.} $ip {,} ip_comma
				set high_byte [expr $current_sock / 256]
				set low_byte [expr $current_sock % 256]
				set final_message "227 Previously in Passive Mode. ${ip_comma},${high_byte},${low_byte}"
				puts $sock $final_message
				::ftpd::Log debug "::ftpd::command::PASV -> $final_message"
				return
			}
        }
        if { [::ftpd::find_sock $sock] == 0 } {
            # I must send back an error
    		puts $sock "500 PASV errror - No socket any more available on the server. Please try later."
		    return
        }
	}

	# So I found a socket, I must send it back
	# I suppose that I have the local IP temporally in a conf
	set ip $::ftpd::ip
	regsub -all {\.} $ip {,} ip_comma
	...
	set high_byte [expr $current_sock / 256]
	set low_byte [expr $current_sock % 256]
	set final_message "227 Entering Passive Mode. ${ip_comma},${high_byte},${low_byte}"
	puts $sock $final_message
	::ftpd::Log debug "::ftpd::command::PASV -> $final_message"
	return
}
...
proc ::ftpd::data_accept {sock args} {
    upvar #0 ::ftpd::$sock pasv_data

    ::ftpd::Log debug "::ftpd::data_accept - sock -> $sock , args -> $args"

    parray pasv_data
    set mother_socket $pasv_data(mother_socket)
    upvar #0 ::ftp::$mother_socket data
    ::ftpd::Log debug "::ftpd::data_accept - entering with $sock - $args"
    parray data
    fconfigure $sock -translation $data(mode)
    fileevent  $sock readable [list ::ftpd::data_read $mother_socket]
    return
}

proc ::ftpd::data_read {sock} {
    upvar #0 ::ftpd::$sock data
	::ftpd::Log debug "::ftpd::data_read - Entering - sock -: $sock"

    if { $data(sock2a) != "" && [eof $data(sock2a)] } {
        close $data(sock2a)
        # Work was done, next time, forget it
        set data(pasv_filename) ""
        return
    }
    # There is something to be read on the input,
    # I read it and then ?
	# No in fact, I should do a fcopy on the channel to output path
	# And then I must get the current filename. What should be the
	# -command ? Maybe no at first.
	#gets $data(sock2a) data_chunk
    if { [info exists data(pasv_filename)] } {

		# begin modif Xav
		# extract the name of the file without the path
		# so that to act like in active mode
		regexp "(/?.*/)*(.*)" $data(pasv_filename) trash path_to_file short_filename
		ftpd::Log note "::ftpd::data_read - short filename is $short_filename"
		unset data(pasv_filename)
		set data(pasv_filename) $short_filename
		# end modif Xav

        ::ftpd::Log debug "::ftpd::data_read - going to write $data(pasv_filename)"
        # is sock2a really OK ?
	    if { $data(pasv_filename) != "" } {
		    if {![catch {::ftpd::Fs append $data(pasv_filename) $data(mode)} f]} {
		        catch { fcopy $data(sock2a) $f -command [list ::ftpd::GetDone $sock $data(sock2a) $f ""] }
			    puts $sock "250 Requested storage okay, completed for $f ($data(pasv_filename))"
				::ftpd::Log debug "->250 Requested storage okay, completed for $f ($data(pasv_filename))"

            } else {
                puts $sock "500 Copy Failed: $data(pasv_filename) $f"
                ::ftpd::Log debug "500 Copy Failed: $data(pasv_filename) $f"
                ::ftpd::FinishData $sock
            }
        } else {
            ::ftpd::Log debug "::ftpd::data_read - no need to copy empty filename"
        }
    }
}
...
# ::ftpd::find_sock --
#
#       Looking for a local free socket on which to listen
#
# Arguments:
#       start : number from which to start
#       end   : number where to end
#
proc ::ftpd::find_sock { sock {start {49152}} {end {65534}} } {
	upvar #0 ::ftpd::$sock data
	set counter 0

	::ftpd::Log debug "::ftpd::find_sock - sock -> $sock - start -> $start - end -> $end"
	variable last_port
	if { $last_port == "" } {
		set last_port $start
	} else {
	...
	::ftpd::Log debug "::ftpd::find_sock - last_port is $last_port"
	# I am going to create a special function for listening.
	# It will include the orginal socket
	# This is hot, but I see no other way to do it.
	# I thought that on calling the waiting procedure, the server
	# would use the good sock number, but it is not the case. Then
	# I try this code generation stuff.
	if { [info commands ::ftpd::data_accept_${sock}] == "" } {
		# I must define the command
		set data_accept_source "proc ::ftpd::data_accept_${sock} {sock2a args} {
		::ftpd::Log debug \"::ftpd::data_accept_${sock} - sock2a -> \$sock2a , args -> \$args\"
		set mother_socket $sock
		upvar #0 ::ftpd::\$mother_socket data
		set data(sock2a) \$sock2a
		set data(sock2) \$sock2a
		fconfigure \$sock2a -translation \$data(mode)
		if { \[fileevent \$sock2a readable\] == \"\" } {
			::ftpd::Log debug \"::ftpd::data_accept_${sock} - creating fileevent for \$sock2a \"
			fileevent  \$sock2a readable \[list ::ftpd::data_read \$mother_socket\]
		}
		return
		}"
	}

	while { [catch {set sock2a [socket -server ::ftpd::data_accept_${sock} $last_port]} error ] } {
		puts "::ftpd::find_sock - trying $last_port error -> $error"
		incr last_port
		if { $last_port >= $end } {
			set last_port $start
		}
		...
		# I must create a counter if I am after 30000
		# I stop with some error
		incr counter
		if { $counter > 30000 } {
			return 0
		}
	...
	if { [info exists data_accept_source] } {
		::ftpd::Log debug "::ftpd::find_sock - generating the code for waiting on the socket"
		::ftpd::Log debug "$data_accept_source"
		# if the procedure previously exists, I do not need to
		# create it again (well I hope, at least).
		eval $data_accept_source
	}

	# So I found finally a free socket, I store it in data(sock2a)
	set data(sock2) $sock2a
	set data(sock2a) $sock2a
	set data(data_sock_nber) $last_port
	set data(pasv_flag) 1

	# And I create a link between the 2 sockets
	upvar #0 ::ftpd::$sock2a pasv_data
	set pasv_data(mother_socket) $sock

	::ftpd::Log debug "::ftpd::find_sock -- listening on $sock2a - port $last_port"
	return $sock2a
}
